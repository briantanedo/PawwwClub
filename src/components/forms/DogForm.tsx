...
import { useCreateDog, useGetUserById, useGetUserHouseholds, useUpdateDog } from '@/lib/react-query/queriesAndMutations';
...
const DogForm = ({ dog, action }: DogFormProps) => {
    ...
    /* const { data: currentUser, isPending: isUserHouseholdsLoading} = useGetUserById(user.id);
    if (!currentUser) {
        return <OrangeLoader />
    };
    const userHouseholds = currentUser.households as Models.Document[]; */
    
    const { data: userHouseholdsData, isPending: isUserHouseholdsLoading } = useGetUserHouseholds(user.id);
    if (!userHouseholdsData) {
        return <OrangeLoader />
    } 
                    ...
                    <Popover>
                            <PopoverTrigger asChild>
                                ...
                                >
                                    {field.value
                                    ? userHouseholds?.find(
                                        (userHousehold) => userHousehold.$id === field.value
                                        )?.name
                                        : "Select household"}
